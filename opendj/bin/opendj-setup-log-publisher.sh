...
: ${instanceRoot=}
localLogPublisherList=

USAGE="	Usage: `basename $0` [ -p localLogPublisherList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
if [ -z ${localLogPublisherList} ]; then
    for theName in "${!OPENDJ_LOG_PUBLISHERS[@]}"; do
        [ -z "${localLogPublisherList}" ] && localLogPublisherList="${theName}" || localLogPublisherList="${localLogPublisherList} ${theName}"
    done
fi

if [ -z "${localLogPublisherList}" ]; then
	echo "Must pass a valid log publsisher list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Log_Publisher ]; then
	echo "Standard_Setup_Log_Publisher was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localLogPublisherList}" ]; then    
        echo "#### Standard_Setup_Log_Publisher ($localLogPublisherList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Log_Publisher
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Log_Publisher ($localLogPublisherList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
