...
: ${instanceRoot=}
localLenValidatorList=

USAGE="	Usage: `basename $0` [ -p localLenValidatorList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
if [ -z ${localLenValidatorList} ]; then
    for theName in "${!OPENDJ_PASSWD_LEN[@]}"; do
        [ -z "${localLenValidatorList}" ] && localLenValidatorList="${theName}" || localLenValidatorList="${localLenValidatorList} ${theName}"
    done
fi

if [ -z "${localLenValidatorList}" ]; then
	echo "Must pass a valid Length Validator list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Length_Validator ]; then
	echo "Standard_Setup_Length_Validator was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    echo "#### Setting Standard_Setup_Length_Validator ($localLenValidatorList) ==> ${localTargetHost}"
    source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Length_Validator
    if [ $? -ne 0 ]; then
        echo "#### ERROR executing Standard_Setup_Length_Validator ($localLenValidatorList) ==> ${localTargetHost}"    
        result=1
        break
    fi
done
