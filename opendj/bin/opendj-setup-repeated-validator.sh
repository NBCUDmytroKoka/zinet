...
: ${instanceRoot=}
localRepeatValidatorList=

USAGE="	Usage: `basename $0` [ -p localRepeatValidatorList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
while getopts hn:I:p: OPT; do
    case "$OPT" in
        ...
        n)
            ...
            ;;
        p)
            localRepeatValidatorList="$OPTARG"
            ;;
        I)
            instanceRoot="$OPTARG"
            ;;
...
if [ -z ${localRepeatValidatorList} ]; then
    for theName in "${!OPENDJ_PASSWD_REPEAT[@]}"; do
        [ -z "${localRepeatValidatorList}" ] && localRepeatValidatorList="${theName}" || localRepeatValidatorList="${localRepeatValidatorList} ${theName}"
    done
fi

if [ -z "${localRepeatValidatorList}" ]; then
	echo "Must pass a valid Repeat Validator list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Repeated_Validator ]; then
	echo "Standard_Setup_Repeated_Validator was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localRepeatValidatorList}" ]; then    
        echo "#### Standard_Setup_Repeated_Validator ($localRepeatValidatorList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Repeated_Validator
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Repeated_Validator ($localRepeatValidatorList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
