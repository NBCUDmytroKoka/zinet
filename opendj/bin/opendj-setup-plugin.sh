...
USAGE="	Usage: `basename $0` [ -p localPluginsList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
if [ -z ${localPluginsList} ]; then
    for theName in "${!OPENDJ_PLUGINS[@]}"; do
        [ -z "${localPluginsList}" ] && localPluginsList="${theName}" || localPluginsList="${localPluginsList} ${theName}"
    done
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Plugin ]; then
	echo "Standard_Setup_Plugin was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localPluginsList}" ]; then    
        echo "#### Standard_Setup_Plugin ($localPluginsList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Plugin
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Plugin ($localPluginsList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
