...
: ${instanceRoot=}
localDictValidatorList=

USAGE="	Usage: `basename $0` [ -p localDictValidatorList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
if [ -z ${localDictValidatorList} ]; then
    for theName in "${!OPENDJ_PASSWD_DICT[@]}"; do
        [ -z "${localDictValidatorList}" ] && localDictValidatorList="${theName}" || localDictValidatorList="${localDictValidatorList} ${theName}"
    done
fi

if [ -z "${localDictValidatorList}" ]; then
	echo "Must pass a valid Dictionary Validator list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Dictionary_Validator ]; then
	echo "Standard_Setup_Dictionary_Validator was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localDictValidatorList}" ]; then    
        echo "#### Standard_Setup_Dictionary_Validator ($localDictValidatorList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Dictionary_Validator
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Dictionary_Validator ($localDictValidatorList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
