...
USAGE="	Usage: `basename $0` [ -p localBackendIdxList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
if [ -z ${localBackendIdxList} ]; then
    for theName in "${!OPENDJ_BACKEND_IDX_CFG[@]}"; do
        [ -z "${localBackendIdxList}" ] && localBackendIdxList="${theName}" || localBackendIdxList="${localBackendIdxList} ${theName}"
    done
fi

if [ -z "${localBackendIdxList}" ]; then
	echo "Must pass a valid backend index list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Rebuild_Backend_Index ]; then
	echo "Standard_Rebuild_Backend_Index was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localBackendIdxList}" ]; then    
        echo "#### Standard_Rebuild_Backend_Index ($localBackendIdxList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Rebuild_Backend_Index
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Rebuild_Backend_Index ($localBackendIdxList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
