...
if [ -z ${localReplicationDomainList} ]; then
    for theName in "${!OPENDJ_REPL_CFG[@]}"; do
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Replication_Domain ]; then
	echo "Standard_Setup_Replication_Domain was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localReplicationDomainList}" ]; then    
        echo "#### Standard_Setup_Replication_Domain ($localReplicationDomainList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Replication_Domain
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Replication_Domain ($localReplicationDomainList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
