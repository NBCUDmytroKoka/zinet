...
: ${instanceRoot=}
localSimilarValidatorList=

USAGE="	Usage: `basename $0` [ -p localSimilarValidatorList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
while getopts hn:I:p: OPT; do
    case "$OPT" in
        ...
        n)
            ...
            ;;
        p)
            localSimilarValidatorList="$OPTARG"
            ;;
        I)
            instanceRoot="$OPTARG"
            ;;
...
if [ -z ${localSimilarValidatorList} ]; then
    for theName in "${!OPENDJ_PASSWD_SIMILAR[@]}"; do
        [ -z "${localSimilarValidatorList}" ] && localSimilarValidatorList="${theName}" || localSimilarValidatorList="${localSimilarValidatorList} ${theName}"
    done
fi

if [ -z "${localSimilarValidatorList}" ]; then
	echo "Must pass a valid Similarity Validator list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Similarity_Validator ]; then
	echo "Standard_Setup_Similarity_Validator was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localSimilarValidatorList}" ]; then    
        echo "#### Standard_Setup_Similarity_Validator ($localSimilarValidatorList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Similarity_Validator
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Similarity_Validator ($localSimilarValidatorList) ==> ${localTargetHost}"
            result=1
            break
        fi
    fi
