...
: ${instanceRoot=}
localStorageSchemeList=

USAGE="	Usage: `basename $0` [ -p localStorageSchemeList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
if [ -z ${localStorageSchemeList} ]; then
    for theName in "${!OPENDJ_PASSWD_STORAGE_SCHEME[@]}"; do
        [ -z "${localStorageSchemeList}" ] && localStorageSchemeList="${theName}" || localStorageSchemeList="${localStorageSchemeList} ${theName}"
    done
fi

if [ -z "${localStorageSchemeList}" ]; then
	echo "Must pass a valid passwd policy config list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Password_Storage_Scheme ]; then
	echo "Standard_Setup_Password_Storage_Scheme was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localStorageSchemeList}" ]; then    
        echo "#### Setting Standard_Setup_Password_Storage_Scheme ($localStorageSchemeList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Password_Storage_Scheme
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Password_Storage_Scheme ($localStorageSchemeList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
