...
: ${instanceRoot=}
localCharValidatorList=

USAGE="	Usage: `basename $0` [ -p localCharValidatorList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
if [ -z ${localCharValidatorList} ]; then
    for theName in "${!OPENDJ_PASSWD_CHARS[@]}"; do
        [ -z "${localCharValidatorList}" ] && localCharValidatorList="${theName}" || localCharValidatorList="${localCharValidatorList} ${theName}"
    done
fi

if [ -z "${localCharValidatorList}" ]; then
	echo "Must pass a valid Character Validator list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Character_Validator ]; then
	echo "Standard_Setup_Character_Validator was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localCharValidatorList}" ]; then    
        echo "#### Standard_Setup_Character_Validator ($localCharValidatorList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Character_Validator
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Character_Validator ($localCharValidatorList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
