...
USAGE="	Usage: `basename $0` [ -p localPolicyCfgList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
if [ -z ${localPolicyCfgList} ]; then
    for theName in "${!OPENDJ_PASSWD_POLICY[@]}"; do
        [ -z "${localPolicyCfgList}" ] && localPolicyCfgList="${theName}" || localPolicyCfgList="${localPolicyCfgList} ${theName}"
    done
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Password_Policy ]; then
	echo "Standard_Setup_Password_Policy was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localPolicyCfgList}" ]; then    
        echo "#### Standard_Setup_Password_Policy ($localPolicyCfgList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Password_Policy
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Password_Policy ($localPolicyCfgList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
