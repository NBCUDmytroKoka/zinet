...
: ${instanceRoot=}
localVirtualAttributeList=

USAGE="	Usage: `basename $0` [ -p localVirtualAttributeList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
while getopts hn:I:p: OPT; do
    case "$OPT" in
        ...
        n)
            ...
            ;;
        p)
            localVirtualAttributeList="$OPTARG"
            ;;
        I)
            instanceRoot="$OPTARG"
            ;;
...
if [ -z ${localVirtualAttributeList} ]; then
    for theName in "${!OPENDJ_VIRTATTRS[@]}"; do
        [ -z "${localVirtualAttributeList}" ] && localVirtualAttributeList="${theName}" || localVirtualAttributeList="${localVirtualAttributeList} ${theName}"
    done
fi

if [ -z "${localVirtualAttributeList}" ]; then
	echo "Must pass a valid virtual attribute list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Virtual_Attribute ]; then
	echo "Standard_Setup_Virtual_Attribute was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localVirtualAttributeList}" ]; then    
        echo "#### Setting Standard_Setup_Virtual_Attribute ($localVirtualAttributeList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Virtual_Attribute
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Virtual_Attribute ($localVirtualAttributeList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
