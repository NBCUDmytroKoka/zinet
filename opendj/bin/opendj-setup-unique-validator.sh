...
: ${instanceRoot=}
localUniqueValidatorList=

USAGE="	Usage: `basename $0` [ -p localUniqueValidatorList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
while getopts hn:I:p: OPT; do
    case "$OPT" in
        ...
        n)
            ...
            ;;
        p)
            localUniqueValidatorList="$OPTARG"
            ;;
        I)
            instanceRoot="$OPTARG"
            ;;
...
if [ -z ${localUniqueValidatorList} ]; then
    for theName in "${!OPENDJ_PASSWD_UNIQUE[@]}"; do
        [ -z "${localUniqueValidatorList}" ] && localUniqueValidatorList="${theName}" || localUniqueValidatorList="${localUniqueValidatorList} ${theName}"
    done
fi

if [ -z "${localUniqueValidatorList}" ]; then
	echo "Must pass a valid Unique Validator list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Unique_Validator ]; then
	echo "Standard_Setup_Unique_Validator was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localUniqueValidatorList}" ]; then    
        echo "#### Standard_Setup_Unique_Validator ($localUniqueValidatorList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Unique_Validator
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Unique_Validator ($localUniqueValidatorList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
