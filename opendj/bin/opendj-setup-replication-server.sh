...
: ${instanceRoot=}
localReplicationDomainList=

USAGE="	Usage: `basename $0` [ -p localReplicationDomainList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
while getopts hn:I:p: OPT; do
    case "$OPT" in
        ...
        n)
            ...
            ;;
        p)
            localReplicationDomainList="$OPTARG"
            ;;
        I)
            instanceRoot="$OPTARG"
            ;;
...
if [ -z ${localReplicationDomainList} ]; then
    for theName in "${!OPENDJ_REPL_SVR_CFG[@]}"; do
        [ -z "${localReplicationDomainList}" ] && localReplicationDomainList="${theName}" || localReplicationDomainList="${localReplicationDomainList} ${theName}"
    done
fi

if [ -z "${localReplicationDomainList}" ]; then
	echo "Must pass a valid replication domain list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Replication_Server ]; then
	echo "Standard_Setup_Replication_Server was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localReplicationDomainList}" ]; then    
        echo "#### Standard_Setup_Replication_Server ($localReplicationDomainList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Replication_Server
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Replication_Server ($localReplicationDomainList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
