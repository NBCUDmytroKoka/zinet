...
USAGE="	Usage: `basename $0` [ -p localBackendList ] [ -n localTargetHostList=$(hostname) ] [ -I instanceRoot ]"
...
if [ -z ${localBackendList} ]; then
    for theName in "${!OPENDJ_BACKEND_CFG[@]}"; do
        [ -z "${localBackendList}" ] && localBackendList="${theName}" || localBackendList="${localBackendList} ${theName}"
    done
fi

if [ -z "${localBackendList}" ]; then
	echo "Must pass a valid backend list"
    exit 0
fi
...
if [ ! -f ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Backend ]; then
	echo "Standard_Setup_Backend was not found in ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig."
    echo $USAGE >&2
    cd ${SAVE_DIR}
    exit 1
fi
...
for localTargetHost in "${targetHostList[@]}"; do
    if [ ! -z "${localBackendList}" ]; then    
        echo "#### Standard_Setup_Backend ($localBackendList) ==> ${localTargetHost}"
        source ${OPENDJ_TOOLS_DIR}/share/opendj-standard-dsconfig/Standard_Setup_Backend
        if [ $? -ne 0 ]; then
            echo "#### ERROR executing Standard_Setup_Backend ($localBackendList) ==> ${localTargetHost}"    
            result=1
            break
        fi
    fi
